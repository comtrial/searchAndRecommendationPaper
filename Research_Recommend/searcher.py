...
from Research_Recommend.utils import (
    ...
    DUPLICATE_INDEX_DIR, DEPARTMENT_INDEX_DIR, COMPANY_INDEX_DIR,
)

ix = open_dir(DUPLICATE_INDEX_DIR)
dix = open_dir(DEPARTMENT_INDEX_DIR)
cix = open_dir(COMPANY_INDEX_DIR)
...
def sort_by_date(search_results):
    now = datetime.datetime.now()
    for i in range(len(search_results['results'])):
        if search_results['results'][i][2] == '1-1-1':
            search_results['results'][i] = [
                search_results['results'][i][0],
                search_results['results'][i][1],
                search_results['results'][i][3],
                search_results['results'][i][4],
            ]
        else:
            ...
            search_results['results'][i] = [
                search_results['results'][i][0],
                search_results['results'][i][1],
                search_results['results'][i][3] + score,
                search_results['results'][i][4],
            ]
...
def filter_by_type(search_results, r_type):
    if r_type == [0]:
        return search_results
...
class SearchEngine:
    ...
    def searching(self, input_word, page_num, data_count, r_type):
        ...
        with ix.searcher() as searcher:
            ...
            q = MultifieldParser(sche_info, ix.schema, group=qparser.OrGroup).parse(kkma_analyze(input_word))
            results = searcher.search(q, limit=None)

            for r in results:
                search_results['results'].append([r['idx'], r['type_code']])

        search_results = filter_by_type(search_results, r_type)
        search_results['data_total_count'] = len(search_results['results'])
        search_results['results'] = search_results['results'][
            (page_num - 1) * data_count:min(search_results['data_total_count'], page_num * data_count)
        ]
    ...
    def department_matcher(self, input_word):
        ...
        with dix.searcher() as searcher:
            searcher = searcher.refresh()
            q = QueryParser('sector', dix.schema, group=qparser.OrGroup).parse(kkma_analyze(input_word))
            results = searcher.search(q, limit=None)

            for r in results:
                if r['department'] not in r_list:
                    r_list.append(r['department'])
                    department_list.append(kkma_analyze(r['department']))
...
class Recommend:
    ...
    def more_like_idx(self, input_idx, data_count):
        ...
        with ix.searcher() as searcher:
            ...
            uquery = MultifieldParser(sche_info, ix.schema, group=qparser.OrGroup).parse(
                kkma_analyze(str(title[0][0]))
            )
            ...
            search_results['data_total_count'] = len(search_results['results'])
            search_results['results'] = search_results['results'][
                0:min(search_results['data_total_count'], data_count)
            ]
            search_results['results'] = result_list(search_results, curs)
    ...
    def recommend_by_company(self, input_idx, page_num, data_count, r_type):
        curs, conn = connect_db()
        ...
        for row in rows:
            company['industry'] = row[0] if row[0] is not None else ''
            company['sector'] = row[1] if row[1] is not None else ''

        if company['industry'] == '' and company['sector'] == '':
            return {'results': [], 'data_total_count': 0}

        industry = kkma_analyze(str(company['industry']) + str(company['sector']))
        department = SearchEngine().department_matcher(industry)
        ...
        search_results = InteractionRecommend().append(
            input_idx, content_idx, score_list, researcher_idx, search_results, curs
        )
        search_results = sort_by_date(search_results)
        search_results = filter_by_type(search_results, r_type)
...
class ResearcherSearch:
    ...
    def recommend_by_researcher(self, idx, data_count):
        ...
        with ix.searcher() as s:
            ...
            uquery = MultifieldParser(sche_info, ix.schema, group=qparser.OrGroup).parse(kkma_analyze(field[0][0]))
            ...
            for r in results:
                if r['researcher_idx'] not in idx_list:
                    ...
                    curs.execute(
                        "Select idx, name, department, research_field from tbl_researcher_data where idx = %s",
                        r['researcher_idx'],
                    )
                    researcher_data = curs.fetchall()
                    search_results['results'].append({
                        'researcher_idx': researcher_data[0][0],
    ...
    def recommend_company_to_researcher(self, researcher_idx, data_count):
        curs, conn = connect_db()
        search_results = {
            'results': [],
            'data_total_count': [],
        ...
        with cix.searcher() as searcher:
            searcher = searcher.refresh()
            c_query = MultifieldParser(
                ["industry", "sector"], cix.schema, group=qparser.OrGroup
            ).parse(kkma_analyze(sector_list[0]))
...
class InteractionRecommend:
    ...
    def append(self, idx, content_idx, score_list, researcher_idx, search_results, curs):
        ...
        record_list = record_list[:min(10, len(record_list))]
        remove_list = list()
        append_list = list()
...
class RecentContent:
    ...
    def recent(self, page_num, data_count, data_type):
        ...
        with ix.searcher() as searcher:
            for i in range(3):
                searcher = searcher.refresh()
                q = QueryParser('image_num', ix.schema).parse(str(i))
                results = searcher.search(q, limit=None)
        ...
        search_results = filter_by_type(search_results, data_type)
